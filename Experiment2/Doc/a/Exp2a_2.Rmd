...
mean(M_pred$mean)
sum(M_pred$mean*grid_area2/(4*pi*6371^2))
```

Then we check the ocean mass change. We need to first identify which grid belongs to the oceans.
```{r oceanmean}
Ocean <- readOGR(dsn = paste0(dd, "WP1-BHM/maps/ne_110m_ocean"), layer = "ne_110m_ocean")
grid_inO <- unlist(over(Ocean, SpatialPoints(coords = ice6g2[, 2:3], proj4string = CRS(proj4string(Ocean))), returnList = TRUE))
mean(M_pred$mean[grid_inO])
```
