...
$$X_{mass} |Y_{GRACE} = X_M | Y_{mass}$$, where $Y_{mass}$ is the GRACE observation after adjusted by GIA.
The GIA contribution and the updated unadjusted mass are both give in mm water equivalence on a 1 degree grid.

First load the GIA data. We use the ICE-6G data and use the same mesh grid as $mass$ to reprensent GIA. To substract the GIA from the unadjusted mass, we need re-order the GIA data set to have the same coordinates and orders as the unadjusted mass.
```{r gia_data}
...
grace_m <- INLA_pred$mean[idx_grace]/grace_sp$area # adjust to be height
grace_u <- INLA_pred$sd[idx_grace]/grace_sp$area^2
...
We first plot the predicted $X_M$ on a 1 degree grid and then the aggregated $X_m$ on the same super mascons grid as the GRACE data. We call $X_m$ the unadjusted mass change.
```{r predict_grace, include=TRUE}
M_pred <- res_M$M_pred

M_pred$mean2 <- ifelse(abs(M_pred$mean) > 10, sign(M_pred$mean)*12, M_pred$mean)
M_pred$u2 <- ifelse(abs(M_pred$u) > 5, 7, M_pred$u)
## plot the mean 
lattice::levelplot(mean2 ~ lon + lat, data = M_pred, aspect = "iso", at = seq(-12, 12, 2),
                     panel = function(x,y,z,...){
                       lattice::panel.levelplot(x,y,z,...)
                       map2 <- map("world2", interior = FALSE, plot = FALSE)
                       lattice::panel.xyplot(x=map2$x, y=map2$y, type = "l", col = "black")
                     },
                     main = "The predicited unadjusted mass change  (mm/yr ewh)", xlab = "longitude", ylab = "latitude")
...
spplot(grace_sp, "pred_mean2", at = seq(-21, 21, 2), main = "The predicted unadjusted mean (mm/yr ewh)")
spplot(grace_sp, "pred_u", main = "The predicted uncertainties (mm/yr ewh)")
```

# Update $X_{mass}$

Then load the updated $X_M$ and remove the $x_GIA$.
```{r grace_gia_remove}
M_pred <- res_M$M_pred
mass_m <- res_M$M_pred$mean - ice6g2$trend
mass_u <- res_M$M_pred$u
mass_pred <- data.frame(lon = res_M$M_pred$lon, lat = res_M$M_pred$lat,
                       mean = mass_m, u = mass_u)

ice6g2$trend2 <- ifelse(abs(ice6g2$trend) > 19, sign(ice6g2$trend) * 20, ice6g2$trend)
mass_pred$mean2 <- ifelse(abs(mass_pred$mean) > 19, sign(mass_pred$mean) * 20, mass_pred$mean)

lattice::levelplot(trend2 ~ x_center + y_center, data = ice6g2, aspect = "iso", at = seq(-20, 20, 4),
                     panel = function(x,y,z,...){
                     ...
                     main = "Global GIA (mm/yr ewh)", xlab = "longitude", ylab = "latitude")

lattice::levelplot(mean2 ~ lon + lat, data = mass_pred, aspect = "iso",at = seq(-20, 20, 4),
                     panel = function(x,y,z,...){
                       lattice::panel.fill(col = "grey")
                       lattice::panel.levelplot(x,y,z,...)
                       map2 <- map("world2", interior = FALSE, plot = FALSE)
                     ...
                     main = "Updated mass seen by GRACE after removing GIA (mm/yr ewh)", xlab = "longitude", ylab = "latitude")

```


We also aggregate the mass data into the super mascon grid. This is a very rough approximation since the GIA data are not ready in the super mascon grid. Proper aggregation should be done in the same way as Maike has done for the GRACE data.
```{r grace_grid}
...
GIA_grace <- res_M$Adata %*% GIA_v

mass_grace <- grace_sp@data$pred_mean - GIA_grace
grace_sp@data$gia <- as.numeric(GIA_grace)
grace_sp@data$mass <- as.numeric(mass_grace)

grace_sp@data$gia2 <- ifelse(abs(grace_sp@data$gia) > 49, sign(grace_sp@data$gia)*50, grace_sp@data$gia )
grace_sp@data$mass2 <- ifelse(abs(grace_sp@data$mass) > 49, sign(grace_sp@data$mass)*50, grace_sp@data$mass )
spplot(grace_sp, "gia", main = "GIA aggreated on the GRACE grid (mm/yr ewh)")
spplot(grace_sp, "mass2", main = "Adjusted mass change agrregated on the GRACE grid (mm/yr ewh)")
```
