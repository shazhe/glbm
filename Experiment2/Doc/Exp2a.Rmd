...
save(ress, file ="Z:/WP1-BHM/Experiment2a/exp2a_ssh.RData")
```
...
A_mass_pred <- inla.spde.make.A(mesh = mesh0, loc = grid_pred, weights = areas)

## Create the estimation and prediction stack
st.est <- inla.stack(data = list(y=grace_sp$mmweq), A = list(A_GRACE_data),
                     effects = list(mass = 1:mass_spde$n.spde), tag = "est")
st.pred <- inla.stack(data = list(y=NA), A = list(A_mass_pred),
                      effects = list(mass=1:mass_spde$n.spde), tag = "pred")

st.pred2 <- inla.stack(data = list(y=NA), A = list(A_GRACE_data),
                      effects = list(mass=1:mass_spde$n.spde), tag = "pred")
stmass <- inla.stack(st.est, st.pred)
stmass2 <- inla.stack(st.est, st.pred2)
```
...
## Fix altimetry errors as they are known
hyper <- list(prec = list(fixed = TRUE, initial = 0))
prec_scale <- c(1/grace_sp$std^2, rep(1, nrow(A_mass_pred)))
prec_scale2 <- c(1/grace_sp$std^2, rep(1, nrow(A_GRACE_data)))
...
res_inla2 <- inla(formula, data = inla.stack.data(stmass2, spde = mass_spde), family = "gaussian",
                 scale =prec_scale2, control.family = list(hyper = hyper),
                 control.predictor=list(A=inla.stack.A(stmass2), compute =TRUE))
```
...
## SSH
mass_m <- INLA_pred$mean[pred_idx] 
mass_u <- INLA_pred$sd[pred_idx]
...
ress_mass <- list(res_inla = res_inla, spde = mass_spde, st = stmass, 
            mesh = mesh0,  mass_pred = mass_pred)

save(ress_mass, file ="Z:/WP1-BHM/Experiment2a/exp2a_mass.RData")
```
...
pars_mass <- marginal_par(res = ress_mass, process = "mass", plot = TRUE)
## The posterior modes
...
## plot the mean 
lattice::levelplot(mean ~ lon + lat, data = alt_pred, aspect = "iso", at = seq(-20, 20, 2),
                     panel = function(x,y,z,...){
                       lattice::panel.levelplot(x,y,z,...)
                       map2 <- map("world2", interior = FALSE, plot = FALSE)
                       lattice::panel.xyplot(x=map2$x, y=map2$y, type = "l", col = "black")
...
## plot the uncertainty
lattice::levelplot(u ~ lon + lat, data = alt_pred, aspect = "iso", at = seq(0, 3, 0.5),
                     panel = function(x,y,z,...){
                       lattice::panel.levelplot(x,y,z,...)
                       map2 <- map("world2", interior = FALSE, plot = FALSE)
                       lattice::panel.xyplot(x=map2$x, y=map2$y, type = "l", col = "black")
