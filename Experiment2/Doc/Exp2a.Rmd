...
```{r alt_data}
library(ncdf4)
alt_nc <- nc_open("Z:/WP3-Ocean/BHMinputs/trend_SSH_CCI_200501_201512.nc")
print(alt_nc)
...
In our exploratory analysis, the correlation length is quite long, therefore if the purpose is estimating the hyper-parameters, then we can choose to have a sparse mesh with resolutiong no large than the correlation length. Here we could use say a triangle size to have approximately 5 degree resolution so that the edge length of the triangles are smaller than 500km on average which is much smaller than 3000km. 
...
spplot(grace_sp, "mmweq", at = seq(-50, 50, 5))
...
Use the polygon data to have rought estimate of the correlation length and variance.
```{r Grace_prior}
...
The GRACE data we have are mass anomalies aggregated into large polygon grid and we call them "super mascons" as they are on larger grid than the original data provdided by the GRACE project. To link the data to the process, we need to integrate the process value over the coressponding polygon. The integration can be done numerically as weighted sum of the grid values within the polygons. The grid values are represented by the GMRF.

A most tricky setting is the scaling factor! Since the projection matrix A approximate can map inputs to outputs on different scales, this can make the computation unstable when the difference in the scales are huge. The scale also determines the scale of the marginal variacen and predicted uncertainties. 

We choose to scale the prediction by $1e4$ for a $100km \times 100km$ area resolution. This approximately correpond to a 1 degree resolution and also at the similar scale of the GRACE data polygon size.
```{r grace_link}
...
$$E(X_{steric} | altimetry, GRACE, GIA) = E(X_{SSH}| altimetry) - E(X_{mass}|GRACE) - x_{GIA} \\
V(X_{steric}| altimetry, GRACE, GIA) = V(X_{SSH}| altimetry) + V(X_{mass}|GRACE)$$
...
steric_data <- na.omit(steric_data)
save(res_SSH, ress_mass, grace_sp, steric_data, "Z:/WP1-BHM/Experiment2a/exp2a_all.RData")
```

Plot the steric map.
```{r plot_steric}
lattice::levelplot(mean ~ lon + lat, data = steric_data, aspect = "iso",  
                     panel = function(x,y,z,...){
                       lattice::panel.levelplot(x,y,z,...)
                       map2 <- map("world2", interior = FALSE, plot = FALSE)
                       lattice::panel.xyplot(x=map2$x, y=map2$y, type = "l", col = "black")
