...
## SSH
mass_m <- INLA_pred$mean[idx_grace] 
mass_u <- INLA_pred$sd[idx_grace]
...
grace_m <- INLA_pred2$mean[idx_grace] 
grace_u <- INLA_pred2$sd[idx_grace]
...
aa <- order(cbind(ice6g$y_center, ice6g$x_center))
ice6g2 <- ice6g[aa,]
```
...
Plot the steric map.
```{r plot_steric}
lattice::levelplot(mean ~ lon + lat, data = steric_data, aspect = "iso",  at =seq(-20, 20, 4),
                     panel = function(x,y,z,...){
                       lattice::panel.levelplot(x,y,z,...)
                       map2 <- map("world2", interior = FALSE, plot = FALSE)
                       lattice::panel.xyplot(x=map2$x, y=map2$y, type = "l", col = "black")
...
lattice::levelplot(u ~ lon + lat, data = steric_data, aspect = "iso",  at =seq(0, 10, 2),
                     panel = function(x,y,z,...){
                       lattice::panel.levelplot(x,y,z,...)
                       map2 <- map("world2", interior = FALSE, plot = FALSE)
                       lattice::panel.xyplot(x=map2$x, y=map2$y, type = "l", col = "black")
