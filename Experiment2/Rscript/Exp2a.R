...
fibo_points <- fiboSphere(N = 660, L0 = TRUE)
...
alt_datas <- alt_data2[sample(1:nrow(alt_data2), 5000),]
v1 <- variogram(trend_ssh ~ 1, alt_datas) # Takes long time!
plot(v1)
...
A_pred <- inla.spde.make.A(mesh = mesh_ssh, loc = mesh_ssh$loc)
