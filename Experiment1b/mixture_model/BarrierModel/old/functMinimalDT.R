...
# Master:
# - functMinimalDT.R in bitbucket repository dtrcode
# Slaves: 
# - Gdrive/public/barriermodel1/functMinimalDT_S.R
# - Dopbox Dolphin project
# - Dropbox Shrimp project
  ...
  
{
  ...
  log.prior = function(n, ntheta, theta)
  {
    ...
    if (priorID == 107) { # individual priors, UC1
      if (is.null(lambdas)) {
        lambda0 = 1
        lambda1 = 1
      } else {
      ...
      # define your own prior on the internal log scale
      
    } else { 
      print('This prior is not defined')
      stop('Prior ID does not correspond to implemented priors')
    }
                                  ...
                                  verbose=F, prior.function = NULL, prior.parameters = c(1,1))
{
  ...
  DT$spde = spde.DT
  DT$DT.precision.new = DT.precision.new
  if (is.null(fixed.ranges)){
    DT$Q  = function(ranges, sigma=1){
      return(DT$DT.precision.new(DT$spde, ranges, sigma))
    }
    ...
    DT$Q  = function(ranges, sigma=1){
      ...
      return(DT$DT.precision.new(DT$spde, internal.ranges, sigma))
    }
