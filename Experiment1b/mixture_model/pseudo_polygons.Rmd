...
file_names <- file_names[-c(1:4,12)]
...
GIA_ensemble$std <- sqrt((rowSums(sapply(GIA_priors, function(x) (x$std)^2)))/10)
```
...
names(alltrend) <- c("Pel6VM5", "Pur6VM5", "SnS1", "SnS3", "SKMOR", "SVv3REF", "SVvLALT", "WnW4", "WnW5", "vdW5")    
...
GIA_sp$trend01 <- abs(GIA_sp$trend) <0.2
GIA_sp$trend02 <- abs(GIA_sp$trend) <0.3
GIA_sp$trend03 <- abs(GIA_sp$trend) <0.4
...
plot(zeroPoly01, col = "blue", main = "zero regions -- threshold = 0.2")
```
...
plot(zeroPoly02, col = "blue", main = "zero regions -- threshold = 0.3")
...
zeropoly_map02 <- ggplot()+ coord_fixed()+ geom_polygon(data = GIA_zeroDF02, aes(x=long, y=lat, group = group, fill = !hole),colour="black") + ggtitle("Threshold = 0.3")
...
map_final02 <- ggplot() + coord_fixed() + geom_polygon(data=world_map, aes(x=long, y=lat, group=group), colour="black", fill = NA) + 
  ...
  ggtitle("Final map of real and pseudo observations -- threhold = 0.3")
...
Now we do the same using threshold value 0.4. First create the admissible polygons.
```{r polygons3}
## Generate the polygons
...
plot(zeroPoly03, col = "blue", main = "zero regions -- threshold = 0.4")
...
zeropoly_map03 <- ggplot()+ coord_fixed()+ geom_polygon(data = GIA_zeroDF03, aes(x=long, y=lat, group = group, fill = !hole),colour="black") + ggtitle("Threshold = 0.4")
...
map_fiboZero03 <- zeropoly_map03 + geom_point(data=fibpoint_in03, aes(x=lon, y=lat), pch=19, 
                                    col = "blue", alpha=0.5)  + ggtitle("fibpoints in the zero polygons -- threshold = 0.4")
map_fiboZero03
```
...
map_fibo03 <- zeropoly_map03 + geom_point(data=fibpoint_in03gps, aes(x=lon, y=lat), pch=19, 
  ...
  geom_point(data=GPS_data, aes(x=lon, y=lat), pch=19, 
                                    col = "red", fill = "red", alpha=0.7) +
  ggtitle("fibpoints in the zero polygons and away from GPS -- threshold = 0.4")
map_fibo03
```
...
map_final03 <- ggplot() + coord_fixed() + geom_polygon(data=world_map, aes(x=long, y=lat, group=group), colour="black", fill = NA) + 
  ...
  ggtitle("Final map of real and pseudo observations -- threshold = 0.4")
...
writeOGR(zeroPoly02, dsn = outpath, layer = "zero03", driver="ESRI Shapefile")
writeOGR(zeroPoly03, dsn = outpath, layer = "zero04", driver="ESRI Shapefile")
```
