...
zeropolys2 <- zeroPolys[polyareas > 250 ] 
...
plot(zeroPoly, col = "blue", main = "zero regions -- threshold = 0.2 (refined) ")
...
Tlonlat$lon <- ifelse(Tlonlat$lon >=0, Tlonlat$lon, Tlonlat$lon + 360)
...
plot(mesh_outPoly, rgl = T, col = "lightblue")
```
...
if(Sys.info()["sysname"] == "Windows"){
  GPSV4b <- read.table("Z:/WP2-SolidEarth/GPS/NGL/BHMinputFiles/GPS_v04b.txt", header = T)
}else if(Sys.info()["sysname"] == "Ubuntu"){
...
A_pred <- inla.spde.make.A(mesh = Mesh_GIA, loc = rbind(GPS_loc, Mesh_GIA$loc, zero_loc))
...
GIA_idx <- pred_idx[(nrow(GPS_data)+1):(length(pred_idx) - 3)]
