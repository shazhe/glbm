...
## set the prior 
#sigma and rho from previous settings
# trho <- Tlognorm(mu_r, v_r)
# tsigma <- Tlognorm(mu_s, v_s)
# also need prior for zero polygon region rho -- assume it is very small
mu_r0 <-50/6371
v_r0 <- (100/6371)
trho0 <- Tlognorm(mu_r0, v_r0)

prior0 <- list(sigma = tsigma, range = rbind(trho0, trho) )
...
prior1 <- list(sigma = tsigma, range = trho)
log.prior <- dt.create.prior.log.norm(prior.param = prior1) 
