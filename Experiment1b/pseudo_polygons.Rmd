...
GIA_ensR <- raster(GIA_ens, layer = 2)
GIA_zeroPoly <- rasterToPolygons(GIA_ensR, fun=function(zero2) {zero2 == TRUE}, dissolve=TRUE ) 
plot(GIA_zeroPoly, col = "red", main = "zero certain regions")
```
...
First, generate vertices that evenly distributed over the entire globe. The distance between the vertices is about the same as the correlation length of the of the residual process, which is about 400-500km apart. This is about spreading 1000 points using the Fibnacci spiral on a sphere.
```{r pseudobs}
...
```
Then remove points that are not in the zero 