### DESCRIPTION ###
## Functions are based on Bakka's barrier models
## Extensions include:
## (1) mesh partiton on sphere
## (2) allow varying sigma in the INLA model
...
dt.polygon.omega <- function (mesh, Omega, globe = FALSE, rd = 5) {
  ...
  Omega.SP.list = list()
  n.Omega <- length(Omega)
  if(globe){
    ## The listed polygons
    for (j in 1:(n.Omega-1)) {
      poly.list = list()
      for (tri in Omega[[j]]){
      ...
      mesh.polys = SpatialPolygons(list(Polygons(poly.list, ID='a')))
